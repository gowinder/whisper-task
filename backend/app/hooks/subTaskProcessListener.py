...
class SubTaskProgressListener(ProgressListener):
    ...
    def on_progress(self, current: Union[int, float], total: Union[int, float]):
        ...
        sub_task_progress = (
            self.sub_task_start + self.sub_task_total * sub_task_progress_frac
        )
        self.base_task_listener.on_progress(
            sub_task_progress, self.base_task_total
        )

    def on_finished(self):
        self.base_task_listener.on_progress(
            self.sub_task_start + self.sub_task_total, self.base_task_total
        )
